...
class CiaoData(BaseData):
    r"""The Ciao dataset is a trust-based recommendation dataset.
    The content of the Ciao dataset includes:

    - ``num_users``: The number of unique users.
    - ``num_products``: The number of unique products.
    - ``rating_data``: The rating data matrix with columns [userid, productid, rating, helpfulness].
    - ``trustnetwork_data``: The trust relations between users with columns [trusting_userid, trusted_userid].

    Args:
        ``data_root`` (``str``, optional): The ``data_root`` has stored the data. Defaults to ``None``.
    """

    def __init__(self, data_root: Optional[str] = None) -> None:
        super().__init__("ciao", data_root)

        # Load data from .mat files (you can modify this to load from other sources if needed)
        self.rating_data = scipy.io.loadmat(data_root + '/rating.mat')['rating']
        self.trustnetwork_data = scipy.io.loadmat(data_root + '/trustnetwork.mat')['trustnetwork']

        # Define properties
        self.num_users = len(
            set(self.rating_data[:, 0]).union(set(self.trustnetwork_data[:, 0]), set(self.trustnetwork_data[:, 1])))
        self.num_products = len(set(self.rating_data[:, 1]))

        # You can add other attributes or methods similar to the provided Cora and CoraBiGraph classes if needed.


# Create an instance for demonstration purposes
ciao_data = CiaoData(data_root='/root/hgnnp/data/ciao')
ciao_data.num_users, ciao_data.num_products
print(ciao_data.num_users, ciao_data.num_products)
print(ciao_data.content())
